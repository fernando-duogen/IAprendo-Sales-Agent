...
# === qualifier.py ===
qualifier_code = r'''"""
QualifierAgent - Qualifica escolas usando Claude Haiku.

Avalia cada escola com score 0-100 baseado em 4 criterios:
- Porte e Infraestrutura (25%)
- Perfil Administrativo (25%)
- Potencial de Inovacao (25%)
- Acessibilidade ao Decisor (25%)

Usage:
    from agents.qualifier import QualifierAgent

    agent = QualifierAgent()
    results = agent.execute(companies)
"""
...
class QualifierAgent(BaseAgent):
    ...
    Attributes:
        prompt_template: Template do prompt de qualificacao.

    Methods:
        execute: Qualifica lista de escolas.
        qualify_school: Qualifica uma escola individual.
    """

    def __init__(self) -> None:
        """Inicializa QualifierAgent com prompt template."""
        super().__init__(agent_name='qualifier')
        self.prompt_template = self._load_prompt()

    def _load_prompt(self) -> str:
        """Carrega template do prompt de qualificacao."""
        prompt_path = Path(__file__).parent.parent / 'prompts' / 'qualification_prompt.txt'
        try:
            return prompt_path.read_text(encoding='utf-8')
        except FileNotFoundError:
            logger.error(
                "Prompt de qualificacao nao encontrado",
    ...
    def execute(
        ...
        **kwargs: Any
        ...
        Args:
            companies: Lista de escolas para qualificar.
            **kwargs: Parametros adicionais.

        Returns:
            Lista de resultados com score, priority e reasoning.
        """
        ...
        logger.info(
            "Qualificacao batch concluida",
            extra={
                'total': len(companies),
                'qualified': len(results),
                'failed': len(companies) - len(results)
            }
        )
    ...
    def qualify_school(self, company: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        Args:
            company: Dados da escola (do banco).

        Returns:
            Dicionario com resultado ou None se falhar.
        """
        company_id = company.get('id')
        ...
        logger.info(
            "Qualificando escola",
            extra={'company_id': company_id, 'school_name': school_name}
        )
        ...
        self._update_company(company_id, updates)
    ...
    def _format_school_data(self, company: Dict[str, Any]) -> str:
        """Formata dados da escola para insercao no prompt."""
        fields = [
            ('Nome', 'name'),
            ('Cidade', 'city'),
            ('UF', 'state'),
            ...
            ('Dependencia Administrativa', 'admin_dependency'),
            ('Etapas de Ensino', 'education_levels'),
            ('Porte', 'school_size'),
            ('Telefone', 'phone'),
            ('Website', 'website'),
    ...
    def _parse_response(
        ...
        try:
            ...
            logger.error(
                "Falha ao parsear resposta do Claude",
                extra={
                    'company_id': company_id,
                    'error': str(e),
