...
def _get_schools(status: str, limit: int, company_ids: Optional[List[str]] = None):
    """Busca escolas por status, opcionalmente filtradas por IDs."""
    if company_ids:
        return db.get_companies_by_ids_and_status(company_ids, status)[:limit]
...
def run_pipeline(
    ...
    if "qualify" in active_steps:
        ...
        to_qualify = _get_schools("raw", qualify_limit, company_ids)
    ...
    if "enrich" in active_steps:
        ...
        to_enrich = _get_schools("qualified", enrich_limit, company_ids)
    ...
    if "contacts" in active_steps:
        ...
        to_find_contact = _get_schools("enriched", enrich_limit, company_ids)
    ...
    if "write" in active_steps:
        ...
        to_write: list = []
        for status in ("enriched", "qualified"):
