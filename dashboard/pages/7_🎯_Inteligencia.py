"""Pagina 12 - Inteligencia ENEM: ranking P1/P2/P3 + explorador livre.

Nova pagina criada pela integracao de school_analytics (migration 015).
Fonte unica de verdade: reusa helpers de agent/tools/enem_tools.py para
garantir que o dashboard respeita as mesmas regras eticas que o IAlex.
"""
import json
import sys
...
try:
    ...
    st.error(
        f"Modulo agent.tools.enem_tools indisponivel: {e}. "
        "Aplique a migration 015 e rode o import_school_analytics.py."
    )
    st.stop()


# ---------------------------------------------------------------------------
# KPIs globais (topo da pagina)
# ---------------------------------------------------------------------------
...
st.title("📊 Inteligencia ENEM")
st.caption(
    "Ranking P1/P2/P3 + Explorador livre de dados analiticos. "
    "Fonte: school_analytics (vintage ENEM 2024). "
    "Todas as metricas respeitam os guardrails eticos do IAlex "
    "(regras de amostra_confiavel, peer group, perfil do municipio)."
)
...
# ---------------------------------------------------------------------------
# Tabs: Ranking + Explorador livre
# ---------------------------------------------------------------------------
tab_ranking, tab_radar, tab_explorer = st.tabs([
    "🎯 Ranking P1/P2/P3",
    "🎯 Radar Comparativo",
    "🧪 Explorador livre",
])
...
with tab_ranking:
    ...
    with fc1:
        uf_filter = st.text_input("UF (ex: RS)", placeholder="deixar vazio = Brasil todo")
    with fc2:
        municipio_filter = st.text_input("Municipio", placeholder="ex: Porto Alegre (parcial)")
    with fc3:
        dep_filter = st.selectbox(
            "Dependencia",
            ["todas", "Privada", "Estadual", "Municipal", "Federal"],
            index=0,
        )
    with fc4:
        prio_filter = st.selectbox("Prioridade", ["todas", "P1", "P2", "P3"], index=0)

    limite = st.slider("Max leads", 10, 200, 50, step=10)

    if st.button("🎯 Rodar ranking", type="primary", use_container_width=True):
        params = {"limite": limite}
        if uf_filter.strip():
            params["uf"] = uf_filter.strip().upper()
        if municipio_filter.strip():
            params["municipio"] = municipio_filter.strip()
        if dep_filter != "todas":
            params["dependencia"] = dep_filter
        if prio_filter != "todas":
            params["prioridade"] = prio_filter

        with st.spinner("Rodando priorizar_leads_enem..."):
            result_raw = _handle_priorizar_leads_enem(params)
        ...
        if "erro" in result:
            ...
            with pk1:
                metric_card(
                    "Total P1 (quentes)",
                    str(result.get("total_p1", 0)),
                    COLORS["error"], icon="local_fire_department",
                )
            with pk2:
                metric_card(
                    "Total P2 (gap)",
                    str(result.get("total_p2", 0)),
                    COLORS["warning"], icon="trending_down",
                )
            with pk3:
                metric_card(
                    "Total P3 (defensivo)",
                    str(result.get("total_p3", 0)),
                    COLORS["secondary"], icon="shield",
                )
            with pk4:
                metric_card(
                    "Retornados",
                    str(result.get("total_retornados", 0)),
                    COLORS["info"], icon="list",
                )

            leads = result.get("leads", [])
            if not leads:
                alert_banner("Nenhum lead com os filtros aplicados.", "info")
            else:
                # Aviso P3 global (se houver)
                p3_avisos = [l for l in leads if l.get("prioridade") == "P3"]
                if p3_avisos:
                    alert_banner(
                        f"⚠️ {len(p3_avisos)} leads P3 (URGENCIA DEFENSIVA) nesta lista. "
                        "Ao gerar emails, o tom DEVE ser sobre movimento do mercado, "
                        "NUNCA atribuir queda a escola individual (regra etica #3).",
                        "warning",
                    )

                st.markdown("")
                section_header(f"Leads ({len(leads)})", "leaderboard")

                # Montar DataFrame
                df_leads = pd.DataFrame([
                    {
                        "Prio": l.get("prioridade"),
                        "Nome": l.get("nome"),
                        "Municipio": l.get("municipio"),
                        "UF": l.get("uf"),
                        "Dep": l.get("dependencia"),
                        "Presentes": l.get("presentes"),
                        "Gap": l.get("gap_vs_peer_2024"),
                        "Trajet. peer": l.get("trajetoria_peer"),
                        "No CRM": "✓" if l.get("esta_em_companies") else "—",
                        "INEP": l.get("inep"),
                    }
                    for l in leads
                ])
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "Dep": st.column_config.TextColumn("Dep", width="small"),
                        "Presentes": st.column_config.NumberColumn("Presentes", width="small"),
                        "Gap": st.column_config.NumberColumn(
                            "Gap peer",
                            format="%+.1f",
                            width="small",
                            help="Gap vs peer em 2024. Negativo = abaixo dos pares.",
                        ),
                        "Trajet. peer": st.column_config.TextColumn(
                            "Trajet. peer",
                            width="medium",
                            help="Trajetoria 5 anos do GRUPO de concorrentes, nao da escola.",
                        ),
                        "No CRM": st.column_config.TextColumn("No CRM", width="small"),
                        "INEP": st.column_config.TextColumn("INEP", width="small"),
                    },
                )

                # Expander por lead com motivo completo
                st.markdown("")
                with st.expander("Ver motivos detalhados por lead"):
                    for l in leads[:30]:
                        prio = l.get("prioridade")
                        nome = l.get("nome")
                        motivo = l.get("motivo")
                        aviso = l.get("aviso_fernando")
                        st.markdown(f"**[{prio}] {nome}** ({l.get('municipio')}/{l.get('uf')})")
                        st.caption(motivo)
                        if aviso:
                            alert_banner(aviso, "warning")
                        st.markdown("---")
                ...
                st.download_button(
                    "📥 Exportar CSV",
                    data=csv,
                    file_name=f"leads_enem_{uf_filter or 'brasil'}_{prio_filter}.csv",
                    mime="text/csv",
                )

                # ---------------------------------------------------------
                # F3 — Importar leads fora do CRM
                # ---------------------------------------------------------
                # Para cada lead com esta_em_companies=False, oferece botao
                # individual + botao bulk. Usa _handle_importar_escola do
                # brain.py via lazy import (so carrega quando clicado).
                leads_fora = [l for l in leads if not l.get("esta_em_companies")]
                if leads_fora:
                    st.markdown("")
                    with st.expander(
                        f"➕ Importar leads fora do CRM ({len(leads_fora)} escolas)",
                        expanded=False,
                    ):
                        st.caption(
                            "Estas escolas estao no school_analytics mas ainda nao "
                            "foram importadas pro CRM. Importar cria um registro em "
                            "`companies` com status='raw' pronto pra qualificacao e "
                            "enriquecimento."
                        )

                        # Bulk import (so se tiver >=3 leads pra valer a pena)
                        if len(leads_fora) >= 3:
                            if st.button(
                                f"🚀 Importar todos os {len(leads_fora)}",
                                key="bulk_import_crm",
                                use_container_width=True,
                                type="primary",
                            ):
                                try:
                                    from agent.brain import _handle_importar_escola  # lazy
                                    progresso = st.progress(0.0, text="Importando...")
                                    resultados = {"ok": 0, "ja_existia": 0, "erro": 0}
                                    for i, l in enumerate(leads_fora):
                                        try:
                                            r = json.loads(
                                                _handle_importar_escola({"inep": l["inep"]})
                                            )
                                            if r.get("sucesso"):
                                                resultados["ok"] += 1
                                            elif r.get("ja_existia"):
                                                resultados["ja_existia"] += 1
                                            else:
                                                resultados["erro"] += 1
                                        except Exception:
                                            resultados["erro"] += 1
                                        progresso.progress(
                                            (i + 1) / len(leads_fora),
                                            text=f"Importando {i+1}/{len(leads_fora)}...",
                                        )
                                    st.success(
                                        f"✓ {resultados['ok']} importadas · "
                                        f"{resultados['ja_existia']} ja existiam · "
                                        f"{resultados['erro']} erros"
                                    )
                                    st.cache_data.clear()
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"Erro carregando importador: {e}")

                        st.markdown("---")

                        # Botao individual por escola
                        for l in leads_fora:
                            c1, c2, c3, c4 = st.columns([3, 2, 1, 2])
                            with c1:
                                st.markdown(f"**{l.get('nome')}**")
                                st.caption(
                                    f"INEP {l.get('inep')} · {l.get('prioridade')}"
                                )
                            with c2:
                                mun = l.get("municipio") or "?"
                                uf = l.get("uf") or "?"
                                st.caption(f"{mun}/{uf}")
                                st.caption(l.get("dependencia") or "")
                            with c3:
                                gap = l.get("gap_vs_peer_2024")
                                if gap is not None:
                            ...
                            with c4:
                                if st.button(
                                    "Importar", key=f"imp_one_{l['inep']}"
                                ):
                                    try:
                                        from agent.brain import _handle_importar_escola  # lazy
                                        r = json.loads(
                                        ...
                                        if r.get("sucesso"):
                                            st.success("✓ Importada")
                                            st.cache_data.clear()
                                            st.rerun()
                                        elif r.get("ja_existia"):
                                            st.info(
                                                r.get("mensagem", "Ja existia no CRM")
                                            )
                                        else:
                                            st.error(
                                                r.get("erro", "Erro desconhecido")
                                            )
                                    except Exception as e:
                                        st.error(f"Erro: {e}")
